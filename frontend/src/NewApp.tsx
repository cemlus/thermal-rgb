...
function NewApp() {
  ...
  const handleUpload = async () => {   
    ...
    if (
      API_URL.includes("YOUR-CLOUDFLARE-URL") ||
      API_URL.includes("YOUR-ID-HERE")
    ) {
    ...
    try {
      ...
      if (err.message === "Network Error") {
        setError(
          "Network Error: Could not reach the spare laptop. Is the Cloudflare tunnel running?"
        );
      } else if (err.response?.status === 413) {
        setError(
          "File too large for Cloudflare free tier (Limit is 100MB, usually fine)."
        );
      } else {
