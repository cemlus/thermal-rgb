...
// ----------------------------------------------------------------------
// 🔗 PASTE YOUR NGROK URL HERE (from the spare laptop terminal)
...
function App() {
  ...
  const handleUpload = async () => {
    ...
    if (NGROK_URL.includes("YOUR-ID-HERE")) {
      setError("⚠️ You forgot to paste your Ngrok URL in App.tsx!");
      return;
    }
    ...
    try {
      ...
      if (err.message === 'Network Error') {
        setError("Network Error: Could not reach the spare laptop. Is Ngrok running?");
      } else {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4 font-sans bg-gray-950 text-white">
      ...
      <div className="mt-8 text-gray-600 text-sm">
        Running on Node.js + ONNX Runtime via Ngrok
      </div>
    </div>
  );
}
